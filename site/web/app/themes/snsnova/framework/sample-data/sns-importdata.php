<?php
if ( ! function_exists( 'snsnova_importdata' ) ) {
	function snsnova_importdata(){
		$msg = '';
		...
		if( $datatype=='widget' ){
	        ...
	        ob_start();
			snsnova_import_widget($widget_data);
			ob_end_clean();
			update_option('sns_imported_widget', 1);
		}
		if( $datatype=='content' ){
			...
			update_option('sns_imported_content',1);
		...
		if( $datatype=='theme' ){
			...
			snsnova_set_themeoptions($option_data);
		}
		ob_end_clean();
		if($datatype == 'widget'){
...
if ( ! function_exists( 'snsnova_set_themeoptions' ) ){
	function snsnova_set_themeoptions($option){
		$option = json_decode($option,true);
		update_option('snsnova_themeoptions',$option);
	}
}
if(!function_exists('snsnova_import_revslider')){
	function snsnova_import_revslider(){
...
if(!function_exists('snsnova_import_widget')){
	function snsnova_import_widget($import_array){
		...
		foreach ( $sidebars_data as $import_sidebar => $import_widgets ) :
			foreach ( $import_widgets as $import_widget ) :
				...
				if ( isset( $wp_registered_sidebars[$import_sidebar] ) ) :
					...
					$new_widget_name 		= snsnova_widget_name( $title, $index );
...
if(!function_exists('snsnova_widget_name')){
	function snsnova_widget_name($widget_name, $widget_index){
