...
jQuery(function ($) {
	...
	$('body').on('change', '#billing_kota, #shipping_kota', function () {
		
		var country			= $("select.country_to_state, input.country_to_state").val(),
			provinsi		= $(this).closest('div').find('#billing_state option:selected, #shipping_state option:selected'),
		...
		if (country === 'ID') {
			...
			$.post(agenwebsite_woocommerce_jne_params.ajax_url, data, function (response) {
				...
				if (fieldKec.is('input')) {
					...
					fieldKec = $('#billing_kota, #shipping_kota').closest('div').find('select#billing_city, select#shipping_city');
				}
